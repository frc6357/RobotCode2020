...
/**
 * A TurnToColorCommandGroup that will extend the lifter, turn on the motor, and
 * rotate the control panel to the color that the field sensor needs to detect.
 */
public class TurnToColorCommandGroup extends SequentialCommandGroup {
    ...
    public TurnToColorCommandGroup(SK20ColorWheel subsystem) {
        addCommands(
                // Extends the mechanism and checks to see if we are under the control panel. If
                // we are
                // then start the motor if not then we have to move closer.
                new SpinnerStartCommand(subsystem),

                // Rotates the control panel to get the right color that the field sensor needs
                // to detect.
